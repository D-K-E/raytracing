...
HittableList random_scene() {
  ...
  shared_ptr<CheckerTexture> checker =
      make_shared<CheckerTexture>(make_shared<SolidColor>(0.2, 0.3, 0.1),
                                  make_shared<SolidColor>(.8, 0.0, 0.2));
      ...
      if ((center - vec3(4, 0.2, 0)).length() > 0.9) {
        if (choose_mat < 0.8) {
          ...
          scene.add(make_shared<MovingSphere>(
              ...
              make_shared<Lambertian>(make_shared<SolidColor>(albedo)) // albedo
              ));
  ...
  scene.add(make_shared<Sphere>(
      point3(-4, 1, 0), 1.0,
      make_shared<Lambertian>(make_shared<SolidColor>(0.5, 0.1, 0.8))));
